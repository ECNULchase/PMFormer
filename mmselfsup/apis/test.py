...
from mmselfsup.apis.eval_pq import PanopticEval
...
def single_semantic_kitti_test(model, data_loader, logger,
    ...
    for i, data in enumerate(data_loader):
        with torch.no_grad():
            macs, params = get_model_complexity_info(model, (116948, 4), as_strings=True, print_per_layer_stat=True)
            ...
            evaluator.addBatch(argmax_algo_1.data.cpu().numpy(), data['points_label'].data[0][0])
            if rank == 0:
...
def multigpu_semantic_kitti_test_pq(model, data_loader, logger,
                                 gpu_collect=True, save_scan=False):
    model.eval()
    results = []
    dataset = data_loader.dataset
    evaluator = PanopticEval(dataset.get_n_classes(),None,
                        dataset.ignore_class,min_points=50)
    acc = AverageMeter()
    iou = AverageMeter()
    progress = 0
    rank, world_size = get_dist_info()
