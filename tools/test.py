...
from mmselfsup.apis.test import single_semantic_kitti_test,multigpu_semantic_kitti_test,submit_to_sever
...
def parse_args():
    ...
    parser.add_argument(
        ...
        'is allowed.')
    parser.add_argument('--num_vote', type=int, default=0)
...
def main():
    ...
    cfg.data.test['num_vote'] = args.num_vote
    ...
    data_loader = build_dataloader(
        dataset,
        samples_per_gpu=args.num_vote,   #cfg.data.samples_per_gpu,
        workers_per_gpu=cfg.data.workers_per_gpu,
        dist=distributed,
        shuffle=False)

    # build the model and load checkpoints
    ...
    if not distributed:
        ...
        outputs = single_semantic_kitti_test(model, data_loader,logger=None)
    else:
        ...
        outputs = multigpu_semantic_kitti_test(model, data_loader)  # dict{key: np.ndarray}
